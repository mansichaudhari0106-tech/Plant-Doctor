...
def get_photo(photo_id: int) -> bytes:
    """Fetch a photo privately — authenticated, owner-only."""
    r = requests.get(f"{API_BASE}/photos/{photo_id}", headers=_headers())
    r.raise_for_status()
    return r.content
