...
.page-header { background:white; padding:20px 32px; border-bottom:1px solid #e8f0e8; margin-bottom:24px; }
...
.checklist-progress-bar { background:#2D6A4F; height:100%; border-radius:8px; }
.divider-text { text-align:center; color:#9ca3af; font-size:13px; margin:12px 0; }

/* Landing page buttons */
.landing-btn button[data-testid="baseButton-primary"] {
    background:#2D6A4F !important; font-size:16px !important;
...
for k, v in {
    ...
    "show_landing": True, "auth_tab": "login"
}.items():
...
def show_photo(photo: dict, width: int = None):
    """Show a photo privately using the authenticated photo endpoint."""
    try:
        img_bytes = api.get_photo(photo["id"])
        if width:
        ...
        st.markdown("""
        <div style="background:#f3f4f6;border-radius:10px;height:140px;
    ...
    with st.spinner("Logging you in with Google..."):
        try:
            ...
            if r.status_code == 200:
                ...
                st.error(f"Google login failed: {detail}")
                st.query_params.clear()
...
# LANDING PAGE
# ══════════════════════════════════════════════════════════════════════════════
if st.session_state.token is None and st.session_state.show_landing:
...
def auth_screen():
    ...
    with col:
        ...
        # Back to landing
        if st.button("← Back to Home", type="secondary", key="back_landing"):
            st.session_state.show_landing = True
            st.rerun()

        st.markdown('<div style="height:12px"></div>', unsafe_allow_html=True)

        default_tab = 0 if st.session_state.auth_tab == "login" else 1
        ...
        with tab_login:
            ...
            if google_url:
                ...
                st.warning("Google login unavailable.")
        ...
        with tab_signup:
            ...
            if google_url:
                ...
                st.warning("Google signup unavailable.")
    ...
    with main_col:
        ...
        if not plants:
            ...
            for i, plant in enumerate(plants[:6]):
                with cols[i % 3]:
                    # Try to show latest photo privately
                    try:
                        photos = api.list_photos(plant["id"])
                        if photos:
                            img_bytes = api.get_photo(photos[-1]["id"])
                            st.image(img_bytes, use_container_width=True)
                        else:
                            st.markdown("""
                            <div style="background:#e8f5e9;height:120px;display:flex;
                                 align-items:center;justify-content:center;font-size:48px;
                                 border-radius:12px 12px 0 0">🌿</div>""", unsafe_allow_html=True)
                    except Exception:
                        st.markdown("""
                        <div style="background:#e8f5e9;height:120px;display:flex;
                             align-items:center;justify-content:center;font-size:48px;
                    ...
                    st.markdown(f"""
                    <div style="background:white;padding:14px;border-radius:0 0 16px 16px;
                         box-shadow:0 1px 4px rgba(0,0,0,0.08);">
...
def page_my_plants(plants):
    back_button()
    st.markdown("""
    <div class="page-header"><h1>🌿 My Plants</h1><p>Manage your plant collection</p></div>
    """, unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
# DIAGNOSE — AI generates questions dynamically
# ══════════════════════════════════════════════════════════════════════════════
def page_diagnose(plant):
    ...
    # ── Clarifying questions chat (AI-generated) ──
    if pending and pending.get("plant_id") == plant["id"]:
        ...
        with left:
            ...
            st.caption("Answering a few quick questions helps me give you the most accurate care plan")
            ...
            for msg in msgs:
                if msg["role"] == "bot":
                    ...
                    <div style="display:flex;gap:10px;align-items:flex-start;margin-bottom:8px">
                        <div style="background:#2D6A4F;color:white;width:32px;height:32px;
                    ...
                    <div style="text-align:right;margin-bottom:8px">
                        <div class="chat-bubble-user" style="display:inline-block">
                            {msg['text']}</div>
                    </div>""", unsafe_allow_html=True)
        ...
        with right:
            ...
            <div class="pd-card">
                ...
                <div style="font-size:18px;font-weight:700;color:#dc2626">
                    {category_icon(diag.get('symptom_category',''))}
                    {(diag.get('symptom_category') or 'Unknown').title()}</div>
                <div style="margin-top:16px;font-size:13px;color:#6b7280;background:#f9fafb;
                     border-radius:10px;padding:12px">
                    💡 These questions were generated by AI specifically based on your plant's
                    symptoms to help create the most accurate care plan.
                </div>
            </div>""", unsafe_allow_html=True)
        return
    ...
    with left:
        ...
        if uploaded:
            ...
            st.markdown("""
            <div style="border:2px dashed #d1d5db;border-radius:12px;padding:48px;
                        text-align:center;background:#fafafa;margin:8px 0">
        ...
        note = st.text_area("Describe the issue", placeholder="Leaves are turning yellow and some are brown...",
                             height=100, key=f"diag_note_{plant['id']}",
                             label_visibility="collapsed")
        st.caption(f"{len(note)}/300")

        if st.button("🔬 Analyze Plant", disabled=uploaded is None, use_container_width=True):
            with st.spinner("🔍 AI is analyzing your plant photo..."):
                result = api.diagnose(plant["id"], uploaded.getvalue(), uploaded.name, note)
            if result["status"] == "clarifying":
                # Questions are AI-generated from the backend
                st.session_state.pending_diagnosis = {
                    "plant_id": plant["id"],
                    "diagnosis": result["diagnosis"],
                    "questions": result["questions"],  # AI-generated
                }
                st.session_state.chat_messages = []
                st.rerun()
            else:
    ...
    with right:
        ...
        <div class="pd-card">
            <div style="font-size:12px;color:#6b7280;line-height:1.7">
                🔒 <strong>Your photos are private.</strong> Only you can see your plant's
                photos and diagnosis history. They are never shared or made public.
            </div>
        </div>""", unsafe_allow_html=True)
...
def page_care_plans(plant):
    ...
    with left:
        ...
        <div class="recovery-banner">
            ...
            <div>
                <div style="font-size:11px;color:#2D6A4F;font-weight:600;text-transform:uppercase">
                    Expected Recovery Time</div>
        ...
        <div class="pd-card">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
                ...
                <div style="font-size:13px;color:#6b7280">
                    {done_count}/{total} &nbsp;<strong style="color:#2D6A4F">{pct}%</strong></div>
            </div>
        ...
        for i, p in enumerate(plans):
            ...
            <div class="pd-card" style="border-left:3px solid {'#2D6A4F' if i==0 else '#e5e7eb'};
                ...
                <div style="font-weight:600;font-size:13px">
                    {'📌 Current Plan' if i==0 else f'Plan {i+1}'}</div>
                <div style="font-size:11px;color:#9ca3af">{p['created_at'][:10]}</div>
                <div style="font-size:12px;color:#6b7280;margin-top:4px">
                    Recovery: {p.get('expected_recovery','—')}</div>
            </div>""", unsafe_allow_html=True)
...
def page_checkin(plant):
    ...
    with right:
        ...
        if len(photos) >= 2:
            ...
            with col1:
                show_photo(photos[-2])
                st.caption(f"Previous · {photos[-2]['taken_at'][:10]}")
            with arrow_col:
                st.markdown("<div style='text-align:center;font-size:24px;margin-top:40px'>→</div>",
                            unsafe_allow_html=True)
            with col2:
                show_photo(photos[-1])
                st.caption(f"Latest · {photos[-1]['taken_at'][:10]}")
...
def page_gallery(plant):
    ...
    <div class="page-header">
        ...
        <p>Your private recovery timeline for <strong>{plant['name']}</strong></p>
    </div>""", unsafe_allow_html=True)
    ...
    <div class="pd-card" style="margin-bottom:16px">
        <div style="display:flex;justify-content:space-between;align-items:center">
            <div style="font-weight:700">{len(photos)} photos · Oldest → Newest</div>
            <div style="font-size:12px;color:#2D6A4F;background:#f0f7f0;
                 padding:4px 12px;border-radius:20px">🔒 Private — only visible to you</div>
        </div>
    </div>""", unsafe_allow_html=True)
