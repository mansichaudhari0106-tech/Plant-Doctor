    ...
    with st.spinner("Logging you in with Google..."):
        try:
            ...
            if r.status_code == 200:
                ...
                # Show the actual error detail from backend
                try:
...
def auth_screen():
    ...
    with col:
        ...
        # Fetch Google URL once for both tabs
        google_url = None
        try:
        ...
        with tab_login:
            ...
            # Google OAuth — use link_button so it actually navigates
            if google_url:
                st.link_button("🔵  Continue with Google", google_url, use_container_width=True)
            else:
                st.warning("Google login unavailable — Supabase not configured.")
        ...
        with tab_signup:
            ...
            if google_url:
                ...
                st.warning("Google signup unavailable — Supabase not configured.")
            ...
            <div style="display:grid;gap:10px">
                <div>
                    ...
                    <div style="font-size:14px;margin-top:2px">
                        {plant.get('species') or 'Auto-detected from photo'}</div>
                </div>
                <div>
                    <div style="font-size:11px;color:#9ca3af;font-weight:600;text-transform:uppercase">
                        Location</div>
                    <div style="font-size:14px;margin-top:2px">{plant.get('location') or '—'}</div>
                </div>
                <div>
                    <div style="font-size:11px;color:#9ca3af;font-weight:600;text-transform:uppercase">
                        Status</div>
...
def page_care_plans(plant):
    ...
    with left:
        ...
        <div class="pd-card">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
                ...
                <div style="font-size:13px;color:#6b7280">
                    {done_count}/{total} &nbsp;
                    <strong style="color:#2D6A4F">{pct}%</strong></div>
            </div>
        ...
        st.markdown("""
        <div style="background:#f0f7f0;border-radius:10px;padding:12px 16px;
                    margin-top:8px;font-size:13px;color:#2D6A4F">
